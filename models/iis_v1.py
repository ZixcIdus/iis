"""Тестовая версия ИИС из раннего документа."""
...
class IISVersion1(BaseIISModel):
    """Ранняя тестовая версия ИИС с базовой четырёхкомпонентной формулой."""

    def calculate_A(self, inputs: dict[str, object], mode: str) -> ComponentResult:
        """Считает A по асимметрии альфа-ритма."""
        ...
        if not np.isfinite(alpha_left) or not np.isfinite(alpha_right):
            return self._component_result(np.nan, "unavailable", "", "A недоступен без α-мощности слева и справа.")

        value = (alpha_left - alpha_right) / (alpha_left + alpha_right + EPSILON)
        provenance = self._merge_provenance([prov_1, prov_2])
        source = self._merge_sources([source_1, source_2])
        return self._component_result(value, provenance, source, "Формула из тестовой версии: A=(αL-αR)/(αL+αR).")
    ...
    def calculate_H(self, inputs: dict[str, object], mode: str) -> ComponentResult:
        ...
        provenance = self._merge_provenance([prov_1, prov_2])
        source = self._merge_sources([source_1, source_2])
        return self._component_result(value, provenance, source, "Формула из тестовой версии: H=DA/50-C/15.")
    ...
    def calculate_Q(self, inputs: dict[str, object], mode: str) -> ComponentResult:
        """Считает диагностический Q для сопоставления с валентностью."""
        ...
        provenance = self._merge_provenance([prov_1, prov_2])
        source = self._merge_sources([source_1, source_2])
        return self._component_result(value, provenance, source, "Диагностический Q добавлен для сравнения с self-report, в ИИС версии 1 не входит.")
    ...
    def calculate_K(
        ...
        """В тестовой версии коэффициент K отсутствовал."""

        return self._component_result(np.nan, "unavailable", "", "В тестовой версии документа коэффициент K отсутствует.")

    def calculate_IIS(self, components: dict[str, ComponentResult], mode: str) -> dict[str, object]:
        """Считает итоговый ИИС по формуле тестовой версии."""

        prepared = self._prepare_score_inputs(components, mode)
        note = "Тестовая версия документа: ИИС=σ(0.35A+0.25Γ+0.30H+0.10V)."
